  ...
  if (startPayload) {
    ...
    try {
      ...
      if (referrer) {
        ...
        await referrer.addReferral(user.telegramId);
...
🎉 New Referral!
 ...
 ${user.fullName} (@${user.username || 'No Username'}) has joined using your referral link!
...
`;
  ...
  await ctx.reply(referralMessage, {
    ...
    ...Markup.inlineKeyboard([
      [
        Markup.button.url('🔗 Copy Referral Link', referralLink),
        Markup.button.callback('🏠 Back to Menu', 'menu')
      ]
    ])
  });
...
Top Earners:
1. User1 - ${config.CURRENCY_SYMBOL}5000
2. User2 - ${config.CURRENCY_SYMBOL}4500
3. User3 - ${config.CURRENCY_SYMBOL}4000
4. User4 - ${config.CURRENCY_SYMBOL}3500
5. User5 - ${config.CURRENCY_SYMBOL}3000

Your Rank: ${user.rank} - ${config.CURRENCY_SYMBOL}${user.balance}
        `;
          ...
          await ctx.editMessageText(taskDetails, {
            ...
            ...Markup.inlineKeyboard([
              [
                ...
                Markup.button.callback('🏠 Back to Tasks', 'tasks')
              ]
            ])
          });
        }
...
Reward: ${config.CURRENCY_SYMBOL}${task.reward}
            `, {
              parse_mode: 'HTML',
              ...Markup.inlineKeyboard([
                [Markup.button.callback('🏠 Cancel', 'tasks')]
              ])
            });
